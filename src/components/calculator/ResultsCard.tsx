...
export function ResultsCard({ result, className }: ResultsCardProps) {
  ...
  const handleDownload = () => {
    ...
    const income = result.input.netProfit || result.input.w2Wages || 0
    const entityType = result.input.filingEntity.replace('_', ' ').toUpperCase()
    
    // Create a readable text format instead of JSON
    const content = `
RETIRERIGHT CALCULATION SUMMARY
Generated: ${date}

═══════════════════════════════════════════════════════════════

PERSONAL INFORMATION
Business Structure: ${entityType}
State: ${result.input.state}
Age: ${result.input.age}
Annual Income: ${formatCurrency(income)}
Contribution Type: ${result.input.contributionType.replace('_', ' ')}
Marginal Tax Rate: ${formatPercentage(result.input.marginalTaxRate)}

═══════════════════════════════════════════════════════════════

OPTIMAL CONTRIBUTION STRATEGY

Employee Contribution: ${formatCurrency(contributions.employeeDeferral)}
${contributions.preTaxAmount > contributions.employerContribution ? `  • Pre-tax: ${formatCurrency(contributions.preTaxAmount - contributions.employerContribution)}` : ''}
${contributions.rothAmount > 0 ? `  • Roth: ${formatCurrency(contributions.rothAmount)}` : ''}
${contributions.catchUpAmount > 0 ? `  • Catch-up (50+): ${formatCurrency(contributions.catchUpAmount)}` : ''}

Employer Contribution: ${formatCurrency(contributions.employerContribution)}
  • Always pre-tax

TOTAL ANNUAL CONTRIBUTION: ${formatCurrency(contributions.total)}
  • ${formatPercentage(contributions.total / income)} of income

═══════════════════════════════════════════════════════════════

TAX SAVINGS ESTIMATE

Annual Tax Savings: ${formatCurrency(taxSavings.estimatedSavings)}
Effective Contribution Cost: ${formatCurrency(taxSavings.effectiveContributionCost)}
...
Employee Deferral Limit: ${formatCurrency(limits.electiveDeferralLimit)}
Overall 415(c) Limit: ${formatCurrency(limits.overallLimit415c)}
Compensation Limit: ${formatCurrency(limits.compensationLimit)}
${limits.catchUpLimit > 0 ? `Catch-up Limit: ${formatCurrency(limits.catchUpLimit)}` : ''}
...
DISCLAIMER
These are planning estimates only and should not be considered as tax or 
legal advice. Consult with a qualified CPA or financial advisor before 
making contribution decisions. IRS limits and tax rules may change.

Generated by RetireRight (retireright.vercel.app)
Created by Connor Pham (cpham2870@sdsu.edu)
    `.trim()
    ...
    link.href = url
    link.download = `RetireRight-Summary-${new Date().toISOString().split('T')[0]}.txt`
